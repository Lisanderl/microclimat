...
#include "lang/EN"
	...
	void events()
	{
		...
		for (uint8_t n = 0; n < MAX_EVENTS; n++)
		{
			...
			if (_events[n].function && currentTime >= _events[n].time)
			{
				...
				debug(n + 1);
				debug(F(") set for "));
				debugln(UTC.dateTime(_events[n].time));
				...
				{
					// if event sceduled in the past, this will exclude multiple execution bug
					if (currentTime - _events[n].time > MINUTELY)
					{
						Serial.println("debug.. in the past ");
						_events[n].time = currentTime + _events[n].next_time;
						return;
					}
					_events[n].time += _events[n].next_time;
				}
			}
		}
		yield();
	...
	void deleteEvent(const uint8_t event_handle)
	{
		if (event_handle && event_handle <= MAX_EVENTS)
		{
			...
			debug(event_handle);
			debug(F("), set for "));
			debugln(UTC.dateTime(_events[event_handle - 1].time));
	...
	time_t makeTime(const uint8_t hour, const uint8_t minute, const uint8_t second, const uint8_t day, const uint8_t month, const uint16_t year)
	{
		...
		tm.Month = month;
		if (year > 68)
		{ // time_t cannot reach beyond 68 + 1970 anyway, so if bigger user means actual years
			tm.Year = year - 1970;
		}
		else
		{
...
{
	eepromBegin();
	if (address + EEPROM_CACHE_LEN > eepromLength())
	{
		triggerError(CACHE_TOO_SMALL);
		return false;
	}
...
#ifdef EZTIME_CACHE_EEPROM
	eepromBegin();
	if (_eeprom_address < 0)
	{
		triggerError(NO_CACHE_SET);
		return;
	}
	for (int16_t n = _eeprom_address; n < _eeprom_address + EEPROM_CACHE_LEN; n++)
		EEPROM.write(n, 0);
	eepromEnd();
#endif

#ifdef EZTIME_CACHE_NVS
	if (_nvs_name == "" || _nvs_key == "")
	{
		triggerError(NO_CACHE_SET);
		return;
	}
...
bool Timezone::writeCache(String &str)
{
	...
	if (year() >= 2018)
		months_since_jan_2018 = (year(LAST_READ) - 2018) * 12 + month(LAST_READ) - 1;
	...
	if (str.length() > MAX_CACHE_PAYLOAD)
	{
		triggerError(CACHE_TOO_SMALL);
		return false;
	}
...
bool Timezone::readCache(String &olson, String &posix, uint8_t &months_since_jan_2018)
{
	...
	for (uint16_t n = _eeprom_address; n < last_byte; n++)
		checksum += EEPROM.read(n);
	checksum += 42;
	if (checksum != EEPROM.read(last_byte))
	{
		eepromEnd();
		return false;
	}
...
{
	defaultTZ = this;
	debug(F("Default timezone set to "));
	debug(_olson);
	debug(F("  "));
	debugln(_posix);
}
...
uint8_t Timezone::setEvent(void (*function)(), const uint8_t hr, const uint8_t min, const uint8_t sec, const uint8_t day, const uint8_t mnth, uint16_t yr, const uint16_t recurrence_time /* ONCE */)
{
	time_t t = ezt::makeTime(hr, min, sec, day, mnth, yr);
	return setEvent(function, t);
}

uint8_t Timezone::setEvent(void (*function)(), time_t t /* = TIME_NOW */, const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */, const uint16_t recurrence_time /* ONCE */)
{
	...
	for (uint8_t n = 0; n < MAX_EVENTS; n++)
	{
		if (!_events[n].function)
		{
			...
			_events[n].time = t;
			_events[n].next_time = recurrence_time;
			debug(F("Set event (#"));
			debug(n + 1);
			debug(F(") to trigger on: "));
			debugln(UTC.dateTime(t));
			return n + 1;
		}
	}
...
String Timezone::militaryTZ(time_t t /*= TIME_NOW */, const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */)
{
	...
	if (o % 60)
		return "?"; // If it's not a whole hour from UTC, it's not a timezone with a military letter code
	o = o / 60;
	if (o > 0)
...
namespace ezt
{
	...
	uint8_t setEvent(void (*function)(), time_t t /* = TIME_NOW */, const ezLocalOrUTC_t local_or_utc /* = LOCAL_TIME */, const uint16_t recurrence_time /* ONCE */) { return (defaultTZ->setEvent(function, t, local_or_utc)); }
	void setTime(const uint8_t hr, const uint8_t min, const uint8_t sec, const uint8_t day, const uint8_t month, const uint16_t yr, const uint16_t recurrence_time /* ONCE */) { defaultTZ->setTime(hr, min, sec, day, month, yr); }
